...
for experiment in ["vanilla", "model"]:
    results_1, time = getdata("results/" + str(experiment), "4")
    ...
    if "vanilla" in experiment:
        ...
        name = "QSS"
    sns.tsplot(
        condition=f"{name}", time=time, data=results_1, color=colors[c], linestyle='-', err_style='ci_bars', interpolate=True, legend=True).set(xlim=(0, 50000))

    c += 1 


plt.rc('legend', fontsize=50)    # legend fontsize
plt.ylabel("Average Success",fontsize=30)
...
plt.show()
plt.savefig(f"windy_cliffworld.png")
