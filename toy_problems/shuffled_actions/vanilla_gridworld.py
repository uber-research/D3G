...
def train():
    for trial in range(50):
        ...
        if args.shuffled:
            vanilla_file = open("results/vanilla_shuffle_" + str(num_actions) + "_"  + str(trial) + ".txt", "w") 
            with open(f'stored_models/qsa/{trial}.txt', 'r') as f:
                Q = eval(f.read())
        else:
            vanilla_file = open("results/vanilla_original_" + str(num_actions) + "_"  + str(trial) + ".txt", "w") 
        ...
        if not args.shuffled:
            with open(f'stored_models/qsa/{trial}.txt', 'w+') as f:
                f.write(str(dict(Q)))
train()
